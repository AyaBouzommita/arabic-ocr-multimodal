...
Note: The JiWER scorer is Aya's primary deliverable (US-07). This
implementation provides Ilyess's working version so the Tesseract
baseline can be measured end-to-end.
...
def evaluate_document(gt_text: str, ocr_text: str) -> Dict:
    ...
    Returns:
        Dictionary with keys: cer, wer, ref_chars, hyp_chars, ref_words, hyp_words.
    """
...
def evaluate_corpus(
    ...
    Returns:
        ...
        ocr_text, cer, wer, avg_confidence, processing_time_ms.
    """
...
def corpus_summary(df: pd.DataFrame) -> Dict:
    ...
    Returns:
        Dictionary with avg_cer, avg_wer, total_documents, total_time_ms.
    """
    return {

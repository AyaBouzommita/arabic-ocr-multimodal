...
Ensures that PaddleOCR produces output matching the frozen
...
class TestOCROutputSchema:
    ...
    def test_valid_result_passes_schema(self):
        ...
        result = OCRResult(
            ...
            engine="paddleocr",
            raw_text="مرحبا",
            tokens=[OCRToken(text="مرحبا", bbox=[10, 20, 100, 50], confidence=95.0)],
    ...
    def test_token_bbox_must_have_four_elements(self):
        ...
        result = OCRResult(
            ...
            engine="paddleocr",
            raw_text="text",
            tokens=[OCRToken(text="word", bbox=[10, 20, 100], confidence=90.0)],
            processing_time_ms=100,
        )
    ...
    def test_from_dict_roundtrip(self):
        ...
        original = OCRResult(
            ...
            engine="paddleocr",
            raw_text="تجربة",
            tokens=[OCRToken(text="تجربة", bbox=[5, 10, 80, 40], confidence=92.3)],
    ...
    def test_word_count_property(self):
        """word_count should return the number of tokens."""
        result = OCRResult(
            document_id="doc_001",
            engine="paddleocr",
            raw_text="مرحبا بالعالم",
    ...
    def test_avg_confidence_property(self):
        """avg_confidence should average all token confidences."""
        result = OCRResult(
            document_id="doc_001",
            engine="paddleocr",
            raw_text="مرحبا بالعالم",
            tokens=[
                OCRToken(text="مرحبا", bbox=[10, 20, 100, 50], confidence=90.0),
                OCRToken(text="بالعالم", bbox=[110, 20, 200, 50], confidence=80.0),
            ],
            processing_time_ms=200,
        )
        assert result.avg_confidence == 85.0

    def test_avg_confidence_no_tokens(self):
        """avg_confidence should return 0.0 when there are no tokens."""
        result = OCRResult(
            document_id="doc_001",
            engine="paddleocr",
            raw_text="",

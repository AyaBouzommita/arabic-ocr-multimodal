...
Validates that the EasyOCREngine:
...
def engine():
    """Create an EasyOCR engine with default Arabic config.

    Uses module scope to avoid re-initializing the heavy model for each test.
    """
    return EasyOCREngine(languages=["ar"])
...
class TestInterfaceContract:
    ...
    def test_engine_name_is_easyocr(self, engine, sample_image):
        """Engine field must be 'easyocr'."""
        result = engine.extract_text(sample_image)
...
class TestEasyOCR:
    """Tests for EasyOCR extraction quality."""

    def test_extracts_arabic_text(self, engine, sample_image):
        """EasyOCR should extract some Arabic text from the sample."""
        result = engine.extract_text(sample_image)
    ...
    def test_extracts_text_from_calligraphy(self, engine, sample2_image):
        """EasyOCR should extract text from the calligraphy sample."""
...
class TestEngineConfig:
    """Tests for EasyOCR engine configuration."""

    def test_engine_name_property(self):
        """engine_name should return 'easyocr'."""
        engine = EasyOCREngine()
        assert engine.engine_name == "easyocr"

    def test_lazy_initialization(self):
        """EasyOCR reader should not be loaded until first use."""
        engine = EasyOCREngine()
        assert engine._reader is None

    def test_custom_languages(self):
        """Custom language list should be stored."""
        engine = EasyOCREngine(languages=["ar", "en"])
        assert engine.languages == ["ar", "en"]

    def test_default_languages(self):
        """Default language should be Arabic."""
        engine = EasyOCREngine()
        assert engine.languages == ["ar"]
